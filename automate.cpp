...
void Etat::setCellule(unsigned int i, unsigned int val) {
    if (i >= taille) throw AutomateException("Erreur Cellule");
    valeur[i] = val;
}
...
{
    this->setCellule(i*taille+j,val);
}
...
void Automate1D::appliquerTransition(const Etat& dep, Etat& dest) const {
    ...
    if (dep.getTaille() != dest.getTaille() || dep.getDim() != dep.getDim()) dest = dep;
    std::vector<unsigned int> conf(voisinage.size());
    for (unsigned int i = 0; i < dep.getTaille(); i++) {
        for(unsigned int j = 0; j < voisinage.size(); j++)
            conf[j] = dep.getCellule(indexTorique(i + voisinage[j],dep.getTaille()));
        dest.setCellule(i, regle[baseNtoBase10(conf,dep.getValMax()+1)]);
    }
}
